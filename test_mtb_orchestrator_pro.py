import pytest
from mtb_orchestrator_pro import GenomicVariantTriageAgent, TrialMatchingAgent, DrugInteractionAgent, MTBCoordinatorPro, main


def test_sub_agents():
    a1 = GenomicVariantTriageAgent()
    alerts1 = a1.evaluate({"metric_primary": 35.0})
    assert len(alerts1) == 1

    a2 = TrialMatchingAgent()
    alerts2 = a2.evaluate({"critical_flag": True})
    assert len(alerts2) == 1

    a3 = DrugInteractionAgent()
    alerts3 = a3.evaluate({"status_text": "DISCORDANT_FINDING"})
    assert len(alerts3) == 1


def test_coordinator():
    coord = MTBCoordinatorPro()
...
def test_domain_registry():
    from mtb_orchestrator_pro import DomainKnowledgeRegistry
